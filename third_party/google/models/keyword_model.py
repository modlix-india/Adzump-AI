...
# KEYWORD INPUT PREDICTION MODELS
...
class KeywordSuggestion(BaseModel):
    ...
    competition: CompetitionLevel = Field(
        default=CompetitionLevel.UNKNOWN, description="Competition level"
    )
    ...
    def roi_score(self) -> float:
        return (
            self.volume / (1 + self.competitionIndex)
            if self.competitionIndex > 0
            else self.volume
        )
...
class OptimizedKeyword(KeywordSuggestion):
    match_type: MatchType = Field(
        default=MatchType.PHRASE, description="Keyword match type"
    )
    rationale: str = Field(default="AI selected", description="Selection rationale")
    is_cross_business: bool = Field(
        default=False, description="Flag for cross-business terms"
    )
    ...
    def validate_cross_business_match_type(self) -> "OptimizedKeyword":
        if self.is_cross_business and self.match_type != MatchType.PHRASE:
            ...
            object.__setattr__(
                self,
                "rationale",
                f"{self.rationale} [Corrected to phrase: cross-business term]",
            )
        return self
...
class NegativeKeyword(BaseModel):
    keyword: str = Field(
        ..., min_length=2, max_length=80, description="Negative keyword text"
    )
...
class KeywordSelectionResponse(BaseModel):
    keywords: List[Dict[str, Any]] = Field(
        default_factory=list, description="Selected keywords"
    )
...
class GoogleNegativeKwReq(BaseModel):
    ...
    positive_keywords: List[OptimizedKeyword] = Field(
        ..., description="List of positive keywords to analyze"
    )


class NegativeKeywordResponse(BaseModel):
    negative_keywords: List[Dict[str, str]] = Field(
        default_factory=list, description="Negative keywords"
    )
...
class KeywordResearchRequest(BaseModel):
    ...
    keyword_type: KeywordType = Field(
        default=KeywordType.GENERIC, description="Type of keywords to generate"
    )
    ...
    seed_count: int = Field(
        default=40, ge=10, le=100, description="Number of seed keywords"
    )
    target_positive_count: int = Field(
        default=30, ge=10, le=100, description="Target positive keywords count"
    )
...
class KeywordResearchResult(BaseModel):
    positive_keywords: List[OptimizedKeyword] = Field(
        default_factory=list, description="Optimized positive keywords"
    )
    brand_info: BusinessMetadata = Field(
        default_factory=BusinessMetadata, description="Business metadata"
    )
    unique_features: List[str] = Field(
        default_factory=list, description="Business unique features"
    )
...
class UpdateKeywordsStrategyRequest(BaseModel):
    ...
    login_customer_id: str = Field(
        ..., description="Google Ads login customer ID (MCC)"
    )
    ...
    data_object_id: str = Field(
        ..., description="Business/data object ID for context extraction"
    )
    ...
    include_negatives: bool = Field(
        False, description="Include negative keywords in analysis"
    )
    include_metrics: bool = Field(
        True, description="Include metrics like impressions, clicks, conversions"
    )
    ...
    language_id: int = Field(
        default=1000,
        description="Language ID for keyword suggestions (default: 1000 for English)",
    )
...
class Keyword(BaseModel):
    ...
    def from_google_row(cls, row: dict) -> "Keyword":
        ...
        cpl = (
            cost_per_conversion_micros / 1_000_000
            if cost_per_conversion_micros > 0
            else None
        )
...
class UpdateKeywordsStrategyResponse(BaseModel):
    ...
    @classmethod
    def create_no_keywords(
        cls, campaign_id: str, keywords: List
    ) -> "UpdateKeywordsStrategyResponse":
    ...
    def create_no_good_keywords(
        cls, campaign_id: str, all_keywords: List, poor_keywords: List[Dict]
    ) -> "UpdateKeywordsStrategyResponse":
    ...
    @classmethod
    def create_error(
        cls, error_msg: str, campaign_id: str = None
    ) -> "UpdateKeywordsStrategyResponse":
...
class FetchKeywordsResponse(BaseModel):
    ...
    keywords_by_ad_group: dict = Field(
        default_factory=dict, description="Keywords grouped by ad_group_id"
    )
