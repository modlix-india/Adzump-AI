        ...
        try:
            ...
            if response.status_code in [429, 500, 503] and attempt < max_attempts - 1:
                ...
                try:
                    ...
                    retry_hint = error_data.get('error', {}).get('details', [{}])[0].get('quotaErrorDetails', {}).get('retryDelay')
                    if retry_hint:
                        wait_time = int(retry_hint.rstrip('s'))
                except (KeyError, ValueError, IndexError):
                    pass  # Fallback to exponential
                
                logger.warning(f"API error {response.status_code}, retrying in {wait_time}s (attempt {attempt + 1}/{max_attempts})")
            ...
            if attempt < max_attempts - 1:
                wait_time = base_delay * (2 ** attempt)
                logger.warning(f"Timeout, retrying in {wait_time}s (attempt {attempt + 1}/{max_attempts})")
                await asyncio.sleep(wait_time)
            else:
                raise
            ...
            if attempt < max_attempts - 1:
                wait_time = base_delay * (2 ** attempt)
                logger.warning(f"Request error, retrying in {wait_time}s (attempt {attempt + 1}/{max_attempts}): {str(e)[:100]}")
                await asyncio.sleep(wait_time)
            else:
                raise
