...
async def fetch_campaign_keywords(
    ...
    logger.info(f"Fetching keywords for campaign {campaign_id} with duration: {duration or 'ALL TIME'}")
    ...
    try:
        ...
        if not results:
            logger.warning(f"No keywords found for duration: {duration or 'ALL TIME'}")
            return FetchKeywordsResponse(
                ...
                total_keywords=0,
                total_ad_groups=0,
                date_range_used=duration or "ALL TIME",
            )
...
def _build_keywords_fetch_query(
    ...
    date_filter = google_ads_utils.format_date_range(duration)
    negative_filter = "" if include_negatives else "AND ad_group_criterion.negative = FALSE"
...
def _parse_and_merge_suggestions(
    ...
    for kw_idea in api_results:
        try:
            ...
            if len(text_norm) < MIN_KEYWORD_LENGTH or len(text_norm.split()) > MAX_KEYWORD_WORDS:
                continue
