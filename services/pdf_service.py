    ...
    try:
        ...
        for i, page in enumerate(reader.pages):
            ...
            logger.debug(
                ...
                component="pdf",
                page=i,
                length=len(page_text) if page_text else 0,
            )
...
def chunk_text(text: str, max_chars=3000) -> list[str]:
    ...
    if text.strip():
        logger.debug(
            "Final chunk created", component="pdf-chunk", length=len(text.strip())
        )
        chunks.append(text.strip())

    logger.info(
        "Text chunking complete", component="pdf-chunk", chunk_count=len(chunks)
    )
    return chunks
...
async def merge_summaries(summaries: list[str]) -> str:
    logger.info(
        "Merging partial summaries", component="pdf-merge", count=len(summaries)
    )

    combined = "\n".join(summaries)
    logger.debug(
        "Combined text prepared", component="pdf-merge", combined_length=len(combined)
    )

    prompt = prompt_loader.format_prompt(
        "business/pdf_merge_summary.txt", summaries=combined
    )
    ...
    try:
        ...
        logger.info(
            "Merged summary parsed",
            component="pdf-merge",
            summary_length=len(final_text),
        )
    except json.JSONDecodeError:
        logger.warning(
            "Failed to parse merge JSON, using raw response", component="pdf-merge"
        )
...
async def process_pdf_from_path(
    ...
    if not records:
        logger.error(
            "No product found for business URL",
    ...
    logger.info(
        "Product found in storage", component="pdf-storage", product_id=product_id
    )
    ...
    if not raw_text.strip():
        ...
        partial_summaries = [await summarize_chunk(chunk) for chunk in chunks]
        pdf_summary = await merge_summaries(partial_summaries)

    logger.info("PDF summary generated", component="pdf")

    assets = record.get("assets", [])
    assets.append({"fileName": source_url, "fileSummary": pdf_summary})
    ...
    if not update_result.success:
        logger.error(
            "Failed updating assets", component="pdf-storage", product_id=product_id
        )
        raise HTTPException(500, "Failed adding PDF summary")

    logger.info(
        "PDF summary added to storage", component="pdf-storage", product_id=product_id
    )
