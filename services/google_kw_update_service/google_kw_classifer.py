...
class KeywordPerformanceClassifier:
    ...
    def classify_keywords_by_performance(
        ...
        zero_impression_keywords = [kw for kw in keywords if kw.impressions == 0]
        ...
        logger.info(
            ...
            zero_impression_count=len(zero_impression_keywords),
            with_data_count=len(keywords_with_data),
        )
        ...
        for keyword in keywords_with_data:
            poor_performance_reasons, is_critical_failure = (
                self._identify_poor_performance_reasons(keyword, median_cpl)
            )

            if len(poor_performance_reasons) >= 2 or is_critical_failure:
                keyword_dict = keyword.model_dump()
    ...
    def _calculate_median_cpl(self, keywords: List[Keyword]) -> float:
        ...
        valid_cpls = [kw.cpl for kw in keywords if kw.cpl is not None and kw.cpl > 0]
        return (
            sorted(valid_cpls)[len(valid_cpls) // 2]
            if valid_cpls
            else config.DEFAULT_MAX_CPL
        )

    def _identify_poor_performance_reasons(
        self, keyword: Keyword, median_cpl: float
    ) -> Tuple[List[str], bool]:
        """Identify reasons why a keyword is performing poorly and detect critical failures."""
        reasons = []
        ...
        return reasons, is_critical_failure
    ...
    def extract_top_performers(
        ...
        scored_keywords = [
            (
                keyword,
                self._calculate_normalized_keyword_score(
    ...
    def _calculate_normalized_keyword_score(
        ...
        impression_score = (keyword.impressions / max_values["impressions"]) * 100
        ctr_score = (keyword.ctr / max_values["ctr"]) * 100
        quality_score = (
            (keyword.quality_score / max_values["quality"] * 100)
        ...
        if keyword.cpl and keyword.cpl > 0:
            ...
            cpl_normalized = (
                (cpl_value - min_cpl) / (max_cpl - min_cpl) if max_cpl > min_cpl else 0
            )
            cpl_score = (1 - cpl_normalized) * 100
        else:
            cpl_score = 50  # Neutral score if no CPL data

        conversion_score = (keyword.conversions / max_values["conversions"]) * 100
        ...
        final_score = (
            ...
            + impression_score * weights["impressions"]
            + conversion_score * weights["conversions"]
        )
