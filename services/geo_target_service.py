...
class GeoTargetService:
    ...
    async def suggest_geo_targets(
        ...
        if interested_locations:
            ...
            logger.info(f"Looking up postal codes for {len(interested_locations)} locations...")
        ...
        logger.info(f"Suggested {len(result.locations)} geo targets (after hierarchical selection)")
        return result
    ...
    async def resolve_locations_batch(
        ...
        if not country_code:
            country_code = self._infer_country_code(area_location)
            logger.info(f"Inferred country_code: {country_code} from area_location: {area_location}")
        ...
        try:
            async with httpx.AsyncClient(timeout=self.HTTP_TIMEOUT) as client:
                ...
                logger.info(f"Google Ads API returned {len(suggestions)} suggestions for {locations}")
    ...
    async def get_postal_code(self, location_name: str, area_location: str = None) -> Optional[str]:
        """
        ...
        url = f"{self.GEOCODING_API_URL}?address={query}&key={self._google_maps_api_key}"
                ...
                for result in data.get("results", []):
                    for component in result.get("address_components", []):
                        if "postal_code" in component.get("types", []):
                            ...
                            logger.info(f"Found postal code for {location_name}: {postal_code}")
                            return postal_code
    ...
    def _infer_country_code(self, area_location: Optional[str]) -> str:
        """
        ...
        # Use Geocoding API to determine country
        url = f"{self.GEOCODING_API_URL}?address={area_location}&key={self._google_maps_api_key}"
        ...
        try:
            ...
            # Synchronous request for country inference (happens once per request)
            import requests
            response = requests.get(url, timeout=5)
            
            if response.status_code != 200:
                logger.warning(f"Geocoding API error: {response.status_code}, defaulting to IN")
                return "IN"
            
            data = response.json()
            
            if data.get("status") != "OK":
                logger.warning(f"Geocoding failed for '{area_location}': {data.get('status')}, defaulting to IN")
                return "IN"
            ...
            for result in data.get("results", []):
                for component in result.get("address_components", []):
                    if "country" in component.get("types", []):
                        ...
                        logger.info(f"Inferred country '{country_code}' from area_location '{area_location}'")
                        return country_code
            
            logger.warning(f"No country found in geocoding result for '{area_location}', defaulting to IN")
            return "IN"
            
        except Exception as e:
            logger.exception(f"Error inferring country code for '{area_location}': {e}, defaulting to IN")
            return "IN"
    ...
    def _is_valid_coordinates(self, coordinates: Optional[Dict]) -> bool:
        ...
        return bool(
            coordinates 
            and coordinates.get("lat") 
            and coordinates.get("lng")
        )
    ...
    def _select_most_specific_tier(
        ...
        # Group by tier
        tier1 = [loc for loc in locations if loc.target_type in ["Postal Code", "Neighborhood", "Sub District"]]
        tier2 = [loc for loc in locations if loc.target_type in ["City", "District", "Department"]]
        ...
        logger.info(f"  Tier 1 (Most specific): {len(tier1)} locations - {[loc.name for loc in tier1]}")
        logger.info(f"  Tier 2 (Medium): {len(tier2)} locations - {[loc.name for loc in tier2]}")
        logger.info(f"  Tier 3 (Broad): {len(tier3)} locations - {[loc.name for loc in tier3]}")
        logger.info(f"  Tier 4 (Very broad): {len(tier4)} locations - {[loc.name for loc in tier4]}")
        ...
        if tier1:
            ...
            logger.info(f"✅ Selected Tier 2 (fallback to cities): {len(tier2)} locations")
            return tier2
        elif tier3:
            logger.info(f"✅ Selected Tier 3 (fallback to state): {len(tier3)} locations")
            return tier3
        elif tier4:
            logger.warning(f"⚠️ Selected Tier 4 (country - very broad, last resort): {len(tier4)} locations")
            return tier4
        else:
    ...
    def _process_suggestions(
        ...
        for idx, suggestion in enumerate(suggestions, 1):
            ...
            # Log each suggestion
            logger.info(f"  [{idx}] {search_term} → {canonical_name} (Type: {target_type})")
            ...
            if area_lower and target_type != "Country" and area_lower not in canonical_name.lower():
                logger.info(f"    ❌ Skipped: Not in area '{area_location}' (found in {canonical_name})")
                continue
