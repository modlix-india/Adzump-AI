...
class ScraperService:
    ...
    async def scrape(self, url: str) -> ScrapeResult:
        ...
        if captcha_error:
            return ScrapeResult(success=False, url=url, warnings=warnings, error=captcha_error)
        
        # STEP 4: Check for bot protection (BLOCK)
        bot_error = self._detect_bot_protection(html)
        if bot_error:
            return ScrapeResult(success=False, url=url, warnings=warnings, error=bot_error)
    ...
    async def _fetch_html(self, url: str, warnings: List[ScrapeWarning]) -> Tuple[Optional[str], Optional[ScrapeResult]]:
        """
    ...
    def _handle_fetch_error(self, url: str, warnings: List[ScrapeWarning], error_str: str) -> ScrapeResult:
    ...
    def _log_success(self, url: str, data: dict, warnings: List[ScrapeWarning]) -> None:
        ...
        for i, embed in enumerate(map_embeds):
            ...
            if coords:
                logger.info(f"[Scraper]   Map {i+1}: lat={coords.get('lat')}, lng={coords.get('lng')}")
            else:
